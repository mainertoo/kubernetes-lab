...
>
> **Live-ops update — 2026-07 (PR #1139, post-ship):** Two corrections to the shipped system running `open_notebook:1.10.0-single`, outside the v1→v12 pre-ship convergence:
> - **`GET /api/commands/jobs` returns `[]` on ON 1.10.0** regardless of queue depth — F8-008's pinned contract no longer holds. The stale-commands probe was therefore blind and `PocketBridgeOpenNotebookCommandStalled` never fired. `open_notebook_stale_commands_total` is now set by the §7.8 periodic scanner from the bridge's own `embed_pending` backlog (recordings stuck past the ~12min window); the dedicated probe + `list_commands_jobs` client method were removed. (§6, §7.8, §8.1, §8.2)
> - **Embeddings run on the DGX Spark (`192.168.1.94:8081`, mxbai), not Mac-Ollama/mac-llama.** ON's embed target lives in SurrealDB (`default_embedding_model` → credential `spark-mxbai-embed`), so the "Mac-Ollama" references below are the *original* design — the live backend is the Spark. Alert text corrected; the shipped `MacLlamaEmbeddingsUnreachable` (a `source_post`-failure alert — the planned black-box `MacOllamaUnreachable` in §8.2 was never implemented) is renamed **`PocketBridgeSourcePostFailing`**. (§8.2)
>
...
| `GET /api/commands/jobs` (v9 F8-008) | optional query: none discovered | **200** array | No pagination params, no server-side age filter. Bridge fetches and **caps at first 200 entries** (sorted by `created` desc client-side); deeper backlog itself is signal of larger problem. Used by §7.8 stale-scanner. ⚠️ **2026-07 (PR #1139): returns `[]` on ON 1.10.0 regardless of query — no longer consumed; the §7.8 scanner now derives stall from the `embed_pending` backlog.** |
...
| `open_notebook_stale_commands_total` | — | NEW v8 (F7-Live-003), repointed 2026-07 (PR #1139) — Gauge of recordings stuck in `embed_pending` past the ~12min embed window. Set by the §7.8 periodic scanner from the bridge's own backlog (the original `/api/commands/jobs` probe returned `[]` on ON 1.10.0). Auto-clears to 0 once the backlog embeds; catches the [[feedback_open_notebook_worker_stuck_pod_restart]] failure mode |
...
| `MacOllamaUnreachable` *(planned — never implemented; the live embed backend is the DGX Spark, not Mac-Ollama. See `PocketBridgeSourcePostFailing` below.)* | Black-box probe from cluster to `mac-ollama.ollama-egress.svc:11434/api/tags` fails > 10m | warning |
...
| `PocketBridgeEmbedStalled` (NEW v8 F7-Live-001) | `increase(open_notebook_embed_stalled_total[15m]) > 0` — a source completed ingest but vector embedding never confirmed within ~12 min poll window (v12 P12-002 wording). Most common cause is a wedged ON `surreal-commands-worker` — `kubectl rollout restart deploy/open-notebook`; otherwise the DGX Spark mxbai endpoint (192.168.1.94:8081) was unreachable during the window. `/admin/replay repair_embeddings_only=true` also recovers | critical |
| `PocketBridgeOpenNotebookCommandStalled` (NEW v8 F7-Live-003) | `open_notebook_stale_commands_total > 0` for > 10m — Open Notebook's surreal-commands-worker is wedged; kubectl rollout restart deploy/open-notebook to clear. *(2026-07/PR #1139: gauge now derived from the `embed_pending` backlog — see §8.1.)* | critical |
| `PocketBridgeSourcePostFailing` (shipped; renamed from `MacLlamaEmbeddingsUnreachable` 2026-07/PR #1139) | `rate(open_notebook_write_total{operation="source_post",result="fail"}[10m]) > 0.1` for 10m — bridge→ON source POSTs failing (ON rejecting/erroring, distinct from the embeddings backend being down, which surfaces as `PocketBridgeEmbedStalled`) | warning |
